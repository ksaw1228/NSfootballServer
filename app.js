...
MongoClient.connect(process.env.MONGO_DB_URL,(err,client) => {
    ...
    app.listen(process.env.PORT,()=>{
        ...
        getData();
        setRealTimeUpdate(seasonStarted,1)
    })
})

function getData() {
  const leagues = ["PL", "BL1", "FL1", "PD", "SA"];
  leagues.map(leagueName=>updateDB(leagueName,season));
}

async function updateDB(leagueName,season){
...
function setRealTimeUpdate(seasonStart,updateCycleHour){
  if(seasonStart === true) setInterval(getData, 1000 * 60 * 60 * updateCycleHour);
}
